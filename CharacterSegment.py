...
def segment_character(path):  # path参数为读取图片的路径
    img_original = cv2.imread(path)  # 读取图片
    img_gray = cv2.cvtColor(img_original, cv2.COLOR_BGR2GRAY)  # 转换了灰度化

    # 形态学操作
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))  # 获得膨胀的kernel
    image = cv2.dilate(img_gray, kernel)  # 膨胀处理
    ...
    for word in words:
        ...
        if (word[3] > (word[2] * 1)) and (word[3] < (word[2] * 4)) and (word[2] > 10) and \
                (word[3] > 0.5 * img_original.shape[0]) and (word[2] > 0.01 * img_original.shape[1]):
            i = i + 1
            split_image = image[word[1]:word[1] + word[3], word[0]:word[0] + word[2]]  # 分割原图片成字符图像
            split_image = cv2.resize(split_image, (25, 40))  # 统一字符图像的尺寸
            split_image = cv2.copyMakeBorder(split_image, 5, 5, 15, 14, cv2.BORDER_CONSTANT)
